...
class SeatSelector{
  ...
  constructor(rows,cols,canvasId){
    ...
    this.canvas.addEventListener('mouseup',(event)=>{
      ...
      // 记录这个起始坐标点
      this.endPoint={x,y}
      // 根据起始点与结束点，整理出一个矩形框对象{x1,y1,x2,y2}  左上角点坐标 右下角点坐标
      let selectRect={
  ...
  draw(){
    ...
    for(let i=0;i<this.rows;i++){
      for(let j=0;j<this.cols;j++){
        ...
        // 根据座位的类型，设置不同的颜色
        if(seat.type==1){  //普通座位，红色
          this.cvs.fillStyle='#a11'
          this.cvs.fillRect(j*this.seatSize+(j+1)*this.margin,
          i*this.seatSize+(i+1)*this.margin,this.seatSize,this.seatSize)
          continue
        }
        if(seat.type==2){  //过道座位，灰色
  ...
  getSeatTemplateJsonString(){
    ...
    for(let i=0;i<this.rows;i++){
      // 判断当前这排有没有不是过道的座位
      let rowData=this.seats[i]  //当前这排数据
